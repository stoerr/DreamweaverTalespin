...
A single-page client-side application that uses OpenAI's Chat Completions API to generate story outlines and chapters
...
2. When prompted, enter your OpenAI API key. The key is stored locally in `localStorage` under the key `chatgpt_api_key`
   and is only used by the browser to call OpenAI's API.
...
6. Use the Play / Pause / Stop buttons to listen to chapters. Autoplay (enabled by default) will advance and read the
   next chapter while the following chapter is generated in the background.

## Notes and Limitations

- There is no server-side component for the story generation — requests to OpenAI are made directly from the browser
